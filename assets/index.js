...
function init() {
  ...
  tl.from(
    logo,
    0.8,
    { delay: 0.5, opacity: 0, autoAlpha: 0, y: 100, ease: "power3.inOut" },
    "logo"
  );
  tl.fromTo(
    circle,
    1,
    { drawSVG: 0 },
    { drawSVG: "100%", autoAlpha: 1, delay: 1, ease: "circ.inOut" },
    "logo"
  );

  tl.to(circle, 0.5, { opacity: 0 }, "text");
  tl.from(
    chars,
    0.5,
    { x: -200, y: 100, opacity: 0, ease: "expo.inOut", stagger: 0.1 },
    "text"
  );

  tl.to(logo, 0.8, { x: -200, scale: 0.5, ease: "expo.inOut" }, "text");
  tl.to(
    [logo, words],
    { y: 200, opacity: 0, delay: 2, stagger: 0.1, ease: "power3.inOut" },
    "text"
  );
  tl.to(leftBar, 1, { x: "-100%", ease: "expo.inOut" }, "bars");
  tl.to(rightBar, 1, { x: "100%", ease: "expo.inOut" }, "bars");
  tl.to(homeMask, 0.1, { delay: 1, css: { display: "none" } }, "bars");
  tl.fromTo(
    header,
    { delay: 2, y: -200, autoAlpha: 1, display: "none" },
    { y: 0, duration: 1.5, display: "block", ease: "power3.inOut" },
    "bars"
  );
...
tl2.from(
  descWords,
  1,
  { opacity: 0, ease: "power3.inOut", y: 50, stagger: 0.1 },
  "imgs"
);
tl2.from(
  imgOne,
...
let tl3 = gsap
  ...
  .from(
    firstProd,
    1,
    { opacity: 0, x: -200, delay: 0.1, ease: "power3.inOut" },
    "newRel"
  )
  .from(
    secProd,
    1,
    { opacity: 0, x: 200, delay: 0.5, ease: "power3.inOut" },
    "newRel"
  )
  .from(
    thirdProd,
    1,
    { opacity: 0, x: -200, delay: 0.9, ease: "power3.inOut" },
    "newRel"
  );
